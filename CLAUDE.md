# Claude Code Project Instructions — strum-kotlin

## Project Overview

This is **strum-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`strum`](https://crates.io/crates/strum).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/strum/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/strum>
...
Reference example: in `serde-kotlin`, upstream
`tmp/serde/serde_core/src/private/mod.rs` re-exports `Result` from `crate::lib::result`.
The Kotlin tracking file lives at
